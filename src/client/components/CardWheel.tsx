...
interface IProps {
  ...
  sort: boolean;
  yourTurn: boolean;
  play: (cardId: number, color?: Uno.Color) => void;
}
...
export class CardWheel extends React.PureComponent<IProps, IState> {
  ...
  componentDidUpdate(prevProps: IProps) {
    ...
    if (this.props.yourTurn && !prevProps.yourTurn && container) {
      ...
      for (const card of this.props.cards) {
        const el = container.querySelector(`.enabled[data-card-id="${card.id}"]`);

        if (el) {
          const dimens = el.getBoundingClientRect();
          const dist = Math.max(bounds.left - dimens.right, dimens.left - bounds.right);
  ...
  render() {
    const cards = this.props.sort ? this.props.cards.slice().sort((a, b) => order(a) - order(b)) : this.props.cards;
    return (
...
export default connect(
  ...
  (dispatch: Dispatch<ClientAppAction<Uno.Spec>>) => ({
    play: (cardId: number, color?: Uno.Color) => dispatch(Uno.Req.actions.playCard({ cardId, color }))
  })
)(CardWheel);
